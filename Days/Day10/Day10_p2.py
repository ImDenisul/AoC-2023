...
with open('X:\\Proiecte\\alt proiect\\imput.txt', 'r') as fisier:
    ...
    hartacopie = [list(x) for x in harta]
    ok=0
    for i in range(len(harta)):
        ...
        for j in range(len(harta[0])):
            if(harta[i][j] == 'S'):
                ...
                break
    print('start=',i,j)
    hartacopie[i][j] = 'L'#!!!!!!!!!!!!!!!!!!!!!! S devine teava potrivita
    Nr = 0
    ...
    while(i<len(harta)-1):
        i+=1
        j=-1
        while(j<len(harta[0])-1):
            j+=1
            if type(harta[i][j])==int:
                # if(j-1 >=0 and j+1 < len(harta[0]) and type(harta[i][j-1])==int and type(harta[i][j+1])==int and  abs(harta[i][j]-harta[i][j+1])==1 and  (abs(harta[i][j]-harta[i][j-1])==1 or harta[i][j-1]==-1 )):
                #     harta[i][j] = -1
                if(hartacopie[i][j]=='L'):
                    harta[i][j] = -1
                    j+=1
                    while(j<len(harta[0]) and hartacopie[i][j]=='-'):
                        harta[i][j] = -1
                        j+=1 
                    if(hartacopie[i][j]=='J'):
                        harta[i][j] = -1
